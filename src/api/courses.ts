...
/*
  REAL API METHODS
  To connect the dashboards to the backend:
  1. Uncomment these two methods.
  2. Comment out the two fake methods below.
...
export async function getStudentCourses(): Promise<Course[]> {
  ...
  if (!response.data.success) {
    throw new Error(response.data.message || 'Could not retrieve student courses.')
  }
...
export async function getLecturerCourses(): Promise<Course[]> {
  const response = await api.get<CoursesApiResponse>('/api/moodle/lecturer/courses')

  if (!response.data.success) {
    throw new Error(response.data.message || 'Could not retrieve lecturer courses.')
  }
